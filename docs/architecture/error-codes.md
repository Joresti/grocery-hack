...
| `SUGGESTION_NOT_PENDING` | 409 | POST /family/suggestions/{id}/accept, POST /family/suggestions/{id}/dismiss | "That suggestion has already been reviewed." | The suggestion is already `accepted` or `dismissed` (keeps accept/dismiss idempotent / race-safe). |
