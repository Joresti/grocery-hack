...
Run this diagnostic step **after navigating to the print-flyer page** but **before scrolling to load all products**. It catches problems early so you don't waste time scrolling a redesigned page.
...
### Scroll loading verification

The skill relies on scrolling to trigger lazy loading. After each scroll, verify the product count increased:

```js
(function() {
  var beforeCount = document.querySelectorAll('[data-testid="product-title"]').length;
  window.scrollTo(0, document.body.scrollHeight);
  return new Promise(function(resolve) {
    setTimeout(function() {
      var afterCount = document.querySelectorAll('[data-testid="product-title"]').length;
      resolve(JSON.stringify({
        beforeScroll: beforeCount,
        afterScroll: afterCount,
        newItemsLoaded: afterCount - beforeCount,
        lazyLoadWorking: afterCount > beforeCount
      }));
    }, 3000);
  });
})()
...
**If `lazyLoadWorking: false` on the first scroll:** The page may have switched to a "Load More" button, pagination, or loads everything upfront. Check for:
...
(function() {
  ...
  return JSON.stringify({
    ...
    totalProductsAlreadyLoaded: document.querySelectorAll('[data-testid="product-title"]').length
  });
})()
```
...
JSON.stringify({
  ...
  expectedPattern: '/print-flyer',
  urlMatch: window.location.href.includes('/print-flyer')
});
```
