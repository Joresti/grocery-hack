# Drift Detection — Fortinos

Run this diagnostic step **after navigating to the flyer page** and **dismissing popups** but **before iterating through pages**. It catches problems early so you don't waste time paginating through a redesigned site.
...
**Expected range:** 30-60 products per page. If 0, hard drift or popup blocking content. If >200, the page may have switched to loading all products at once (skip pagination).
...
Fortinos is a Loblaw Digital property. Loblaw tends to do platform-wide redesigns that affect fortinos.ca, nofrills.ca, loblaws.com, and other banners simultaneously. If you detect drift here, check the other Loblaw stores too.

## Content Drift

### Temporal coherence

Check whether the flyer data is current by looking for date ranges on the page:
...
- `salePrice` null >60%: expected — Fortinos shows all flyer products, many are regular-price-only
- `wasPrice` null >80%: normal — not all sale items show the "was" price
- `brand` null >50%: normal — many products don't list a separate brand
...
**Expected range:** 1,500–2,500 products across all pages (~49 per page, ~48 pages). If total is <100, pagination is broken. If >5,000, the page may be showing all store products, not just flyer items.
...
After navigating to page 2, verify the product count reset (new products loaded, not the same ones):
...
## Fortinos-Specific Drift Risks

1. **Pagination removal:** Loblaw Digital could switch Fortinos to infinite scroll (like No Frills) or a "Load More" button. If `[aria-label="Next Page"]` disappears, check for these alternatives before reporting drift.

2. **Chakra UI swap:** The `.chakra-linkbox` card container is a Chakra UI class. If Loblaw migrates to a different component library, this selector breaks — but the `data-testid` selectors should survive since they're framework-agnostic.

3. **PC Optimum restructuring:** The points text ("500 PC Optimum Points") is extracted via free text search within the card, not a dedicated selector. If the points UI changes to a badge or icon, the text match will miss it.

4. **Loblaw-wide platform redesign:** Loblaw Digital maintains a shared platform. A redesign that affects No Frills will likely also affect Fortinos. Check both stores when drift is detected at either one.

5. **Store selection gating:** Fortinos may start requiring a store to be selected before showing flyer items. The pickup modal dismissal step handles the current implementation, but a hard gate (no products until store is selected) would require adding store selection logic.
